...
// void removeLessThan( x )     --> Remove item less than x
// void prune( )                --> Remove leaves
// Comparable next( x )         --> Return next item of x
...
class BinarySearchTree
{
  public:
    ...
    void removeMax( );                              // Remove bigger item
    int size( );                                    // Return size tree
    void removeLessThan( const Comparable & x );    // Remove item less than x
    void prune( );                                  // Remove leaves
    Comparable next( const Comparable & x );        // Return next item of x
    // FIN PRACTICA 2
  ...
  protected:
    ...
    //PRACTICA 2
    virtual void removeMax( Node * & t ) const;
    virtual int size( Node * & t ) const;
    virtual void removeLessThan( const Comparable & x, Node * & t ) const;
    virtual void prune( Node * & t ) const;
    Node * next( const Comparable & x, Node * & t ) const;
    //FIN PRACTICA 2
...
class BinarySearchTreeWithRank : public BinarySearchTree<Comparable>
{
  public:
    ...
    int size( )
      { BinarySearchTree<Comparable>::size( ); }
